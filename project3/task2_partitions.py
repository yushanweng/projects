...
# (user_id,[bus_id])
case1 =temp_rdd.filter(lambda x: 'user_id,business_id' not in x).map(lambda x: x.split(','))\
    .map(lambda x: (x[0], x[1])).map(lambda x: (x[0], [x[1]])).reduceByKey(lambda x, y: x + y).filter(lambda x:len(x[1])>k)  # find out user_id that who reviewed more than k businesses

new_rdd=case1.map(lambda x:x[0]).collect()  # get those user_id

# (business_id, user_id(filtered))
son1_rdd = temp_rdd.filter(lambda x: 'user_id,business_id' not in x).map(lambda x: x.split(',')).filter(lambda x:x[0] in new_rdd).map(lambda x: (x[1],x[0])).distinct()\
    .map(lambda x: (x[0],[x[1]])).reduceByKey(lambda x,y:x+y)  # (business_id:[user1,user2, ...])
